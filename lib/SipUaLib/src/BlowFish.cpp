...
void SKYLIB hexToChar(unsigned char *inHexBuf, int inLen, char * outbuf, int outbuflen)
{
    int i;
	unsigned char t,y1,y2,yy[5];
	memset( outbuf, 0, outbuflen);
	for( i = 0; i < inLen ; i++)
	{
		t = inHexBuf[i];
		sprintf(outbuf+i*2, "%02x", t);
		y1 = (unsigned char)outbuf[i*2];
		y2 = (unsigned char)outbuf[i*2+1];
		...
		if( y2 >= 'a' && y2<='f')
			...
			y2 = y2 - '0';
		sprintf((char*)&yy[0],"%02x", y1*16+y2);
	}
}

void SKYLIB charToHex(char *inbuf, int inbuflen, unsigned char *outHexBuf, int outLen)
{
	int i;
	unsigned char y1,y2;
	memset( outHexBuf, 0, outLen);
	for( i = 0; i < inbuflen ; i++)
	{
		y1 = (unsigned char)inbuf[i*2];
		y2 = (unsigned char)inbuf[i*2+1];
		if( y1 >= 'a' && y1<='f')
			y1 = y1 - 87;
		else if( y1 >= '0' && y1 <= '9')
			y1 = y1 - '0';
